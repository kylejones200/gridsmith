...
def run(
    ...
    try:
        if pipeline == "ami-anomaly":
            ...
            typer.echo("  - temperature-load: Temperature-to-load modeling")
            typer.echo("  - load-forecasting: Load forecasting")
            typer.echo("  - predictive-maintenance: Predictive maintenance")
            typer.echo("  - outage-prediction: Outage prediction")
